"""
Tyler Durden Telegram Bot - OpenAI Library Version

Улучшения:
- Использование официальной библиотеки openai
...
Стоимость запросов:
Цены в коде (строки ~119-121) для gpt-4o-mini примерные.
Актуальные цены проверяй на: https://proxyapi.ru/pricing
Текущий курс доллара обнови в переменной usd_to_rub (строка ~122)
"""
...
# Инициализация OpenAI клиента для работы через ProxyAPI
client = OpenAI(
    api_key=PROXYAPI_KEY,
    base_url=PROXYAPI_URL
)
...
async def send_to_chatgpt(messages: list) -> str:
    """Отправка запроса к ChatGPT через OpenAI библиотеку и ProxyAPI"""
    ...
    try:
        ...
        response = client.chat.completions.create(
            model="gpt-4o-mini",  # Модель (можно менять)
            messages=messages,
        ...
        # Цены для gpt-4o-mini (проверь актуальные на proxyapi.ru)
        input_price_per_1m = 0.150  # USD за 1M input токенов
        output_price_per_1m = 0.600  # USD за 1M output токенов
        ...
        raise Exception(f'Не удалось получить ответ от ChatGPT: {str(e)}')
